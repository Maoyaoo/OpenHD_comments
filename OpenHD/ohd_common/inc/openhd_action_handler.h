...
class ArmingStateHelper {
   public:
     ...
     */
    /**
     * 注册一个监听器，当解锁状态发生变化时（例如：解锁、上锁、解锁、...），
     * 该监听器会被可靠地调用。
     * @param tag 需要是一个唯一的标识符（针对所有子模块）
     */
...
class FCRcChannelsHelper {
   public:
    ...
    // default
    // 每次从飞行控制器（FC）接收到 RC 通道值的 MAVLink 数据包时调用
    // （无论任何通道是否有实际变化）。
...
<<<<<<< HEAD
   public:
    ...
    // 同时检查 2G 和 5G 频道会花费很长时间，但在极少数情况下可能是用户需要的。
    // 同时检查 20MHz 和 40MHz（而不是仅检查其中一种）也会使扫描时间翻倍。
...
<<<<<<< HEAD
   public:
    ...
    // 定期广播的摄像头统计信息 / 信息
    // 由摄像头流媒体实现设置 - 由 OHDMainComponent 读取
    // （通过 MAVLink 广播）。简单的读写模式（互斥锁有点过度，但我们没有原子结构体）。
    struct CamInfo {
