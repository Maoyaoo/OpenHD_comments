...
// 这段代码定义了一个静态函数 toggle_led，用于根据传入的文件路径和状态（开或关）来控制 LED 的亮灭。
// 它使用了一个缓存机制来检查指定文件是否有效，并通过写入文件的内容来改变 LED 的状态。
...
//  Linux 系统中，LED 的亮灭通常是通过修改 /sys/class/leds/ 路径下的文件来实现的。文件的 brightness 值决定了 LED 的状态，1 表示亮，0 表示灭。
...
// 实现了 LEDManager 类的单例模式。通过静态成员函数 instance() 返回一个唯一的 LEDManager 实例，
// 确保在程序的整个生命周期内只有一个 LEDManager 实例，避免不必要的多次实例化。
