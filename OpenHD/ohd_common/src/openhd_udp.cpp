...
openhd::UDPForwarder::UDPForwarder(std::string client_addr1, int client_udp_port1) : client_addr(std::move(client_addr1)), client_udp_port(client_udp_port1) {
...
void openhd::UDPForwarder::forwardPacketViaUDP(const uint8_t* packet, const std::size_t packetSize) const {
    ...
    const auto ret = sendto(sockfd, packet, packetSize, 0, (const struct sockaddr*)&saddr, sizeof(saddr));
    if (ret < 0 || ret != packetSize) {
        get_console()->warn("Error sending packet of size:{} to {}:{} code:{} {}", packetSize, client_addr, client_udp_port, ret, strerror(errno));
    }
}
...
openhd::UDPReceiver::UDPReceiver(std::string client_addr, int client_udp_port, openhd::UDPReceiver::OUTPUT_DATA_CALLBACK cb) : mCb(cb) {
...
void openhd::UDPReceiver::loopUntilError() {
    ...
    while (receiving) {
        ...
        if (message_length > 0) {
            ...
            if (receiving) {
                if (std::chrono::steady_clock::now() - m_last_receive_error_log >= std::chrono::seconds(3)) {
                    get_console()->warn("Got message length of: {} log_skip_count:{}", message_length, m_last_receive_error_log_skip_count);
...
// 通过 UDP 协议将数据包发送到指定的目标 IP 地址和端口
void openhd::UDPReceiver::forwardPacketViaUDP(const std::string& destIp, const int destPort, const uint8_t* packet, const std::size_t packetSize) const {
    // set up the destination
    ...
    const auto ret = sendto(mSocket, packet, packetSize, 0, (const struct sockaddr*)&saddr, sizeof(saddr));
    // 发送过程中出现错误，会通过控制台输出警告信息
    if (ret < 0 || ret != packetSize) {
        get_console()->warn("Error sending packet of size:{} to {}:{} code:{} {}", packetSize, destIp, destPort, ret, strerror(errno));
    }
}
