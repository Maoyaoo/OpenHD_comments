...
static std::string createSwEncoder(const CameraSettings& settings) {
    ...
    if (settings.streamed_video_format.videoCodec == VideoCodec::H264) {
        ...
        ss << fmt::format(
            ...
            settings.h26x_bitrate_kbits, settings.h26x_keyframe_interval, settings.h26x_intra_refresh_type < 0 ? "false" : "true", slices_str);
#endif
...
static std::string createDummyStream(const CameraSettings& settings) {
    ...
    ss << fmt::format("video/x-raw, format=I420,width={},height={},framerate={}/1 ! ", settings.streamed_video_format.width, settings.streamed_video_format.height,
 ...
 */
static std::string createRpicamsrcStream(const int camera_number, const CameraSettings& settings, const bool hdmi_to_csi_workaround_half_bitrate) {
    ...
    int bitrateBitsPerSecond = openhd::kbits_to_bits_per_second(settings.h26x_bitrate_kbits);
    ...
    if (camera_number == -1) {
        ss << fmt::format("rpicamsrc name=rpicamsrc bitrate={} preview=0 ", bitrateBitsPerSecond);
    } else {
        ss << fmt::format("rpicamsrc name=rpicamsrc camera-number={} bitrate={} preview=0 ", camera_number, bitrateBitsPerSecond);
    }
    ...
    if (settings.streamed_video_format.videoCodec == VideoCodec::H264) {
        if (settings.force_sw_encode) {
            ...
            ss << fmt::format("video/x-raw, width={}, height={}, framerate={}/1 ! ", settings.streamed_video_format.width, settings.streamed_video_format.height,
                              settings.streamed_video_format.framerate);
            ss << createSwEncoder(settings);
        } else {
            ss << fmt::format(
...
static int rpi_calculate_number_of_mbs_in_a_slice(int frame_height_px, int n_slices) {
    ...
    openhd::log::get_default()->debug("frame_height_px:{} n_slices:{} frame_mb_rows:{} slice_row_mb:{}", frame_height_px, n_slices, frame_mb_rows, slice_row_mb);
    return slice_row_mb;
}
...
static std::string create_rpi_v4l2_h264_encoder(const CameraSettings& settings) {
    ...
    const int bitrateBitsPerSecond = openhd::kbits_to_bits_per_second(settings.h26x_bitrate_kbits);
...
static std::string createLibcamerasrcStream(const CameraSettings& settings) {
    ...
    if (openhd::validate_rpi_libcamera_ev_value(settings.rpi_libcamera_ev_value) && settings.rpi_libcamera_ev_value != RPI_LIBCAMERA_DEFAULT_EV) {
...
static std::string create_veye_vl2_stream(const CameraSettings& settings, const std::string& v4l2_device_name) {
    ...
    ss << fmt::format("video/x-raw,format=(string)UYVY, width={}, height={}, framerate={}/1 ! ", settings.streamed_video_format.width, settings.streamed_video_format.height,
                      settings.streamed_video_format.framerate);
    if (settings.streamed_video_format.videoCodec == VideoCodec::H264) {
        ss << create_rpi_v4l2_h264_encoder(settings);
    } else {
...
static std::string createAllwinnerEncoderPipeline(const CameraSettings& settings) {
    std::stringstream ss;
    assert(settings.streamed_video_format.videoCodec == VideoCodec::H264);
    ss << "sunxisrc name=sunxisrc bitrate=" << settings.h26x_bitrate_kbits << " keyint=" << settings.h26x_keyframe_interval << " ! ";
    return ss.str();
}
 ...
 */
static std::string create_qualcomm_camera1_stream(const int device_index, const CameraSettings& settings) {
    std::stringstream ss;
    int bitrateBitsPerSecond = openhd::kbits_to_bits_per_second(settings.h26x_bitrate_kbits);
    const int bps = openhd::kbits_to_bits_per_second(settings.h26x_bitrate_kbits);
    int qcomIntraRefreshMode = (settings.h26x_intra_refresh_type == -1) ? 0 : settings.h26x_intra_refresh_type + 1;
    const int rotation = get_rotation_degree_qcom(settings);
    ss << fmt::format("qtiqmmfsrc camera={} ! ", device_index);
    ss << fmt::format("video/x-raw, format=NV12, width={}, height={}, framerate={}/1 ! ", settings.streamed_video_format.width, settings.streamed_video_format.height,
                      settings.streamed_video_format.framerate);
    ss << "qtic2venc ";
    ss << "control-rate=1 ";
    ss << "rotate=" << rotation << " ";
 ...
 */
static std::string createV4l2SrcRawAndSwEncodeStream(const std::string& device_node, const CameraSettings& settings) {
    std::stringstream ss;
 ...
 */
/*创建将原始 h264/h265/mjpeg 从 gstreamer 转换为 rtp 的管道部分。
@param videoCodec 要为其创建 rtp 的视频编码格式。
@return gstreamer 管道部分。 */
...
static std::string create_caps_nal(const VideoCodec& videoCodec, bool alignment_nal) {
    if (videoCodec == VideoCodec::H264) {
        if (alignment_nal) {
...
// 假设在编码步骤之后，管道中有一个名为 "t" 的 tee 命令，
// 这样我们可以从中获取原始的编码数据。
// .avi 格式支持 h264 和 mjpeg，即使流崩溃也能正常工作，
// 但不支持 h265。
// .mp4 格式在崩溃时总是会损坏。
// .mkv 格式支持 h264 和 h265，但不支持 mjpeg。
// 由于 .mkv 是 OBS 中的默认格式，
// 因此我们决定在 gst 管道未正确停止的情况下，
// 对 h264 和 h265 使用 .mkv，对 mjpeg 使用 .avi。
...
static std::string create_input_custom_udp_rtp_port(const CameraSettings& settings) {
    ...
    std::stringstream ss;
    ss << fmt::format("udpsrc address={} port={} {} ! ", address, input_port, gst_create_rtp_caps(settings.streamed_video_format.videoCodec));
...
static std::string create_nvidia_encoder_pipeline(const CameraSettings& settings) {
    std::stringstream ss;
    ...
    const auto bitrateBitsPerSecond = openhd::kbits_to_bits_per_second(settings.h26x_bitrate_kbits);
    if (settings.streamed_video_format.videoCodec == VideoCodec::H264) {
