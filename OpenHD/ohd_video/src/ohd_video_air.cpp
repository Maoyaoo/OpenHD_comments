...
OHDVideoAir::OHDVideoAir(std::vector<XCamera> cameras, std::shared_ptr<OHDLink> link) : m_link_handle(std::move(link)) {
    ...
    if (m_generic_settings->get_settings().enable_audio != OPENHD_AUDIO_DISABLE) {
        ...
        auto audio_cb = [this](const openhd::AudioPacket& audioPacket) { on_audio_data(audioPacket); };
    ...
    openhd::LinkActionHandler::instance().action_request_bitrate_change_register(
        [this](openhd::LinkActionHandler::LinkBitrateInformation lb) { this->handle_change_bitrate_request(lb); });
    ...
    openhd::ExternalDeviceManager::instance().register_listener(
        [this](openhd::ExternalDevice external_device, bool connected) { start_stop_forwarding_external_device(external_device, connected); });
...
void OHDVideoAir::configure(const std::shared_ptr<CameraHolder>& camera_holder) {
    ...
    auto frame_cb = [this](int stream_index, const openhd::FragmentedVideoFrame& fragmented_video_frame) { this->on_video_data(stream_index, fragmented_video_frame); };
...
std::array<std::vector<openhd::Setting>, 2> OHDVideoAir::get_all_camera_settings() {
...
std::vector<openhd::Setting> OHDVideoAir::get_generic_settings() {
    ...
    if (!OHDPlatform::instance().is_x20()) {
        ...
        ret.push_back(openhd::Setting{"AUDIO_ENABLE", openhd::IntSetting{m_generic_settings->get_settings().enable_audio, cb_audio}});
    }
    return ret;
}
...
void OHDVideoAir::on_video_data(int stream_index, const openhd::FragmentedVideoFrame& fragmented_video_frame) {
    ...
    if (m_has_localhost_forwarding_enabled) {
        ...
        // 代码会遍历 fragmented_video_frame 中的所有 RTP 分片（rtp_fragments），并通过 UDP 将每个分片的数据发送出去，调用 forwardPacketViaUDP 方法进行转发。
        for (auto& fragment : fragmented_video_frame.rtp_fragments) {
            forwarder->forwardPacketViaUDP(fragment->data(), fragment->size());
        }
        // 如果有脏帧，则将脏帧切割成多个分片，并通过 UDP 进行转发
...
void OHDVideoAir::on_audio_data(const openhd::AudioPacket& audio_packet) {
    ...
    if (m_has_localhost_forwarding_enabled) {
        m_audio_forwarder->forwardPacketViaUDP(audio_packet.data->data(), audio_packet.data->size());
    }
}
...
std::vector<XCamera> OHDVideoAir::discover_cameras() {
    ...
    auto global_settings_holder = std::make_unique<AirCameraGenericSettingsHolder>();  // 创建默认相机配置
    ...
    for (int i = 0; i < num_active_cameras; i++) {
        ...
        if (is_usb_camera(cam_type)) {
            ret.push_back(XCamera{cam_type, i, usb_cam_bus_names[usb_cameras_offset]});  // usb相机
            usb_cameras_offset++;
        } else {
...
bool OHDVideoAir::x_set_camera_type(bool primary, int cam_type) {
    ...
    if (primary) {
        if (!is_valid_primary_cam_type(cam_type))
            return false;
    } else {
        if (!is_valid_secondary_cam_type(cam_type))
            return false;
    }
    if (primary) {
    ...
    if (primary) {
        if (OHDPlatform::instance().is_rpi() && is_rpi_csi_camera(cam_type) || OHDPlatform::instance().is_rock() && is_rock_csi_camera(cam_type)) {
