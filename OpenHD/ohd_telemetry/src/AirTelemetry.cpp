...
AirTelemetry::AirTelemetry() : MavlinkSystem(OHD_SYS_ID_AIR) {
    ...
    if (OHDPlatform::instance().is_rpi()) {
        // 如果当前平台是树莓派，则创建一个 GPIOControl 对象，用于管理 GPIO 控制。
    ...
    // 9. 初始化 UART
    setup_uart();
    // 使用日志对象输出调试信息，表示 AirTelemetry 对象创建成功。
...
void AirTelemetry::send_messages_fc(std::vector<MavlinkMessage>& messages) {
    // 调用 split_into_generic_and_local_only 函数，将 messages 分割为两部分：
    // generic: 通用消息，可以发送到飞控。
    // local_only: 仅本地处理的消息，不需要发送到飞控。
...
void AirTelemetry::send_messages_ground_unit(std::vector<MavlinkMessage>& messages) {
    if (m_wb_endpoint) {
        ...
        for (auto& msg : messages) {
            ...
            if (msg_id == MAVLINK_MSG_ID_PARAM_EXT_VALUE || msg_id == MAVLINK_MSG_ID_PARAM_VALUE) {
...
void AirTelemetry::add_settings_camera_component(int camera_index, const std::vector<openhd::Setting>& settings) {
...
std::vector<openhd::Setting> AirTelemetry::get_all_settings() {
    ...
    auto c_fc_uart_baudrate = [this](std::string, int value) {
        if (!SerialEndpoint::is_valid_linux_baudrate(value))
            return false;
    ...
    ret.push_back(openhd::Setting{air::FC_UART_CONNECTION_TYPE, openhd::StringSetting{m_air_settings->get_settings().fc_uart_connection_type, c_fc_uart_connection_type}});
    ret.push_back(openhd::Setting{air::FC_UART_BAUD_RATE, openhd::IntSetting{static_cast<int>(m_air_settings->get_settings().fc_uart_baudrate), c_fc_uart_baudrate}});
    ret.push_back(openhd::Setting{air::FC_UART_FLOW_CONTROL, openhd::IntSetting{static_cast<int>(m_air_settings->get_settings().fc_uart_flow_control), c_fc_uart_flow_control}});
    ret.push_back(openhd::Setting{air::FC_BATT_N_CELLS, openhd::IntSetting{static_cast<int>(m_air_settings->get_settings().fc_battery_n_cells), c_fc_battery_n_cells}});
    ...
    if (openhd::tmp::file_air_or_ground_exists()) {
        auto c_config_boot_as_air = [](std::string, int value) { return openhd::tmp::handle_telemetry_change(value); };
...
void AirTelemetry::setup_uart() {
    ...
    using namespace openhd::telemetry;
    const auto uart_linux_fd = serial_openhd_param_to_linux_fd(m_air_settings->get_settings().fc_uart_connection_type);
    if (uart_linux_fd.has_value()) {
        ...
        m_fc_serial->configure(options, "fc_ser", [this](std::vector<MavlinkMessage> messages) { this->on_messages_fc(messages); });
    } else {
        m_fc_serial->disable();
    }
}
...
void AirTelemetry::set_link_handle(std::shared_ptr<OHDLink> link) {
    ...
    m_wb_endpoint->registerCallback([this](std::vector<MavlinkMessage> messages) { on_messages_ground_unit(messages); });
}
