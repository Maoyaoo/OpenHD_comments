...
static MavlinkMessage heartbeat(const int sys_id = 255, const int comp_id = 0) {
    MavlinkMessage msg{};
    mavlink_msg_heartbeat_pack(sys_id, comp_id, &msg.m, MAV_TYPE_HELICOPTER, MAV_AUTOPILOT_GENERIC, MAV_MODE_GUIDED_ARMED, 0, MAV_STATE_ACTIVE);
    return msg;
}
static MavlinkMessage position(const int sys_id = 255, const int comp_id = 0) {
    MavlinkMessage msg{};
    float position[6] = {};
    mavlink_msg_local_position_ned_pack(sys_id, comp_id, &msg.m, microsSinceEpoch(), position[0], position[1], position[2], position[3], position[4], position[5]);
    return msg;
}
...
static MavlinkMessage createHeartbeat(const int sys_id, const int comp_id) {
    ...
    mavlink_msg_heartbeat_pack(sys_id, comp_id, &heartbeat.m, MAV_TYPE_GENERIC, MAV_AUTOPILOT_GENERIC, MAV_MODE_GUIDED_ARMED, 0, MAV_STATE_ACTIVE);
    return heartbeat;
}
...
static void debugMavlinkPingMessage(const mavlink_message_t msg) {
    ...
    std::stringstream ss;
    ss << "Ping[" << "sys_id:" << (int)msg.sysid << " comp_id:" << (int)msg.compid << " seq:" << ping.seq << " target_system:" << (int)ping.target_system
...
static MavlinkMessage rc_channels_override_from_array(const int sys_id,
                                                      const int comp_id,
...
// 该函数的作用是从 MAVLink 消息中提取目标系统 ID 和目标组件 ID。
// 支持的消息类型包括：
// COMMAND_LONG
// PARAM_EXT_SET
// PARAM_EXT_REQUEST_READ
// PARAM_EXT_REQUEST_LIST
// 如果消息类型不匹配，则返回 {0, 0}，表示广播消息。
...
// message(s)
// 我们希望实现的优化是“消费”具有给定目标系统 ID 和组件 ID 的消息（例如 OpenHD 地面单元），
// 而不是将它们转发到所有端点（例如飞控，这些端点无论如何都不会对给定消息做任何处理）。
// 形象地说，我们将给定的消息填充到两个“桶”中（generic 和 local_only）。
static std::tuple<std::vector<MavlinkMessage>, std::vector<MavlinkMessage>> split_into_generic_and_local_only(const std::vector<MavlinkMessage>& messages,
