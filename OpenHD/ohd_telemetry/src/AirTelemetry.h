 ...
 */
/**
 * OpenHD 空中遥测。假设在地面树莓派上运行着一个地面实例。
 */
class AirTelemetry : public MavlinkSystem {
   public:
     ...
     */
    /**
     * Telemetry 将在其自己的线程中无限运行，直到 terminate 被设置为 true。
     * @param enableExtendedLogging 是否启用详细的日志记录。
     */
     ...
     */
    /**
     * 在飞行单元上，我们使用 MAVLink 来更改相机设置。每个相机恰好有一个 MAVLink 参数服务器。
     * @param camera_index 0 表示主相机，1 表示副相机，...
     * @param settings 该相机的设置
     */
     ...
     */
    /**
     * 链接句柄可以在实例化之后设置——在设置之前，与地面单元之间的消息将被丢弃。
     */
