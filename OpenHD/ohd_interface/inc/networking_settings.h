...
// 默认情况下处于开启状态，当飞控（FC）处于武装状态时会被禁用，
// 若飞控解除武装则会重新启用。
static constexpr auto WIFI_HOTSPOT_AUTO = 0;        // 热点模式为自动
...
class NetworkingSettingsHolder : public openhd::PersistentSettings<NetworkingSettings> {
   ...
   private:
    [[nodiscard]] std::string get_unique_filename() const override { return "networking_settings.json"; }
    [[nodiscard]] NetworkingSettings create_default() const override { return NetworkingSettings{}; }
