...
WBLink::WBLink(OHDProfile profile, std::vector<WiFiCard> broadcast_cards)
    : m_profile(std::move(profile)), m_broadcast_cards(std::move(broadcast_cards)), m_recommended_max_fec_blk_size_for_this_platform(get_fec_max_block_size_for_platform()) {
    ...
    m_settings = std::make_unique<openhd::WBLinkSettingsHolder>(m_profile, m_broadcast_cards);
    ...
    m_tx_header_2 = std::make_shared<RadiotapHeaderTxHolder>();
    {
        ...
        auto tmp_params = RadiotapHeaderTx::UserSelectableParams{tx_channel_width,  settings.wb_enable_short_guard, settings.wb_enable_stbc, settings.wb_enable_ldpc, mcs_index,
                                                                 set_flag_tx_no_ack};
        m_console->debug("{}", RadiotapHeaderTx::user_params_to_string(tmp_params));
        m_tx_header_1->thread_safe_set(tmp_params);
        auto tmp_params2 = RadiotapHeaderTx::UserSelectableParams{20, settings.wb_enable_short_guard, settings.wb_enable_stbc, settings.wb_enable_ldpc, mcs_index, true};
        m_tx_header_2->thread_safe_set(tmp_params2);
    }
    m_wb_txrx = std::make_shared<WBTxRx>(tmp_wifi_cards, txrx_options, m_tx_header_2);
    m_wb_txrx->m_fatal_error_cb = [this](int error) { on_wifi_card_fatal_error(); };
    ...
    }
    {
        ...
        if (m_profile.is_air) {
            ...
            m_wb_audio_tx = std::make_unique<WBStreamTx>(m_wb_txrx, options_audio_tx, m_tx_header_1);
        } else {
            // we receive video
            auto cb1 = [this](const uint8_t* data, int data_len) { on_receive_video_data(0, data, data_len); };
            auto cb2 = [this](const uint8_t* data, int data_len) { on_receive_video_data(1, data, data_len); };
            auto cb_audio = [this](const uint8_t* data, int data_len) { on_receive_audio_data(data, data_len); };
    ...
    if (m_profile.is_ground()) {
        ...
        m_management_air = std::make_unique<ManagementAir>(m_wb_txrx, m_settings->get_settings().wb_frequency, m_settings->get_settings().wb_air_tx_channel_width);
    ...
    std::function<bool(int)> cb_analyze = [this](int channels_to_scan) { return request_start_analyze_channels(channels_to_scan); };
    openhd::LinkActionHandler::instance().wb_cmd_analyze_channels = cb_analyze;
    if (m_profile.is_air) {
        // MCS is only changed on air
        auto cb_channel = [this](const std::array<int, 18>& rc_channels) { m_rc_channel_helper.set_rc_channels(rc_channels); };
...
bool WBLink::request_set_frequency(int frequency) {
    ...
    // 验证频率变更是否合法
    // 使用 openhd::wb::validate_frequency_change 检查频率是否符合硬件和配置要求
    ...
    auto work_item = std::make_shared<WorkItem>(
        ...
        [this, frequency]() {
            ...
            m_settings->persist();  // 持久化保存设置

            // 如果当前设备是 Air（空中端），执行额外操作
            if (m_profile.is_air) {
        ...
        std::chrono::steady_clock::now());  // 设置工作项的时间戳为当前时间
...
bool WBLink::request_set_tx_power_rtl8812au(int tx_power_index_override, bool armed) {
    m_console->warn("request_set_tx_power_rtl8812au {}index", tx_power_index_override);
...
bool WBLink::request_start_scan_channels(openhd::LinkActionHandler::ScanChannelsParam scan_channels_params) {
    auto work_item = std::make_shared<WorkItem>("SCAN_CHANNELS", [this, scan_channels_params]() { perform_channel_scan(scan_channels_params); }, std::chrono::steady_clock::now());
    return try_schedule_work_item(work_item);
}

bool WBLink::request_start_analyze_channels(int channels_to_scan) {
    auto work_item = std::make_shared<WorkItem>("ANALYZE_CHANNELS", [this, channels_to_scan]() { perform_channel_analyze(channels_to_scan); }, std::chrono::steady_clock::now());
    return try_schedule_work_item(work_item);
}
...
void WBLink::apply_txpower() {
    ...
    if (m_profile.is_air) {
        if (m_broadcast_cards.at(0).type == WiFiCardType::OPENHD_RTL_88X2AU && pwr_index > 50 && settings.wb_air_tx_channel_width == 40) {
...
std::vector<openhd::Setting> WBLink::get_all_settings() {
    ...
    const auto settings = m_settings->get_settings();
    auto change_freq = openhd::IntSetting{(int)settings.wb_frequency, [this](std::string, int value) { return request_set_frequency(value); }};
    change_freq.get_callback = [this]() { return m_settings->unsafe_get_settings().wb_frequency; };
    ret.push_back(Setting{WB_FREQUENCY, change_freq});
    if (m_profile.is_air) {
        // MCS is only changeable on air
        auto change_wb_air_mcs_index = openhd::IntSetting{(int)settings.wb_air_mcs_index, [this](std::string, int value) { return request_set_air_mcs_index(value); }};
        ret.push_back(Setting{WB_MCS_INDEX, change_wb_air_mcs_index});
        // Channel width is only changeable on the air
        auto change_wb_channel_width =
            openhd::IntSetting{(int)settings.wb_air_tx_channel_width, [this](std::string, int value) { return request_set_air_tx_channel_width(value); }};
        change_wb_channel_width.get_callback = [this]() { return m_settings->unsafe_get_settings().wb_air_tx_channel_width; };
        ret.push_back(Setting{WB_CHANNEL_WIDTH, change_wb_channel_width});
        auto cb_change_video_fec_percentage = [this](std::string, int value) { return set_air_video_fec_percentage(value); };
        ...
        auto cb_enable_wb_video_variable_bitrate = [this](std::string, int value) { return set_air_enable_wb_video_variable_bitrate(value); };
        ...
        ret.push_back(Setting{WB_QP_MAX, openhd::IntSetting{(int)settings.wb_qp_max, cb_wb_qp_max}});
        ...
        ret.push_back(Setting{WB_QP_MIN, openhd::IntSetting{(int)settings.wb_qp_min, cb_wb_qp_min}});

        auto cb_wb_max_fec_block_size_for_platform = [this](std::string, int value) { return set_air_max_fec_block_size_for_platform(value); };
        ret.push_back(Setting{WB_MAX_FEC_BLOCK_SIZE_FOR_PLATFORM, openhd::IntSetting{(int)settings.wb_max_fec_block_size, cb_wb_max_fec_block_size_for_platform}});
        auto cb_wb_video_rate_for_mcs_adjustment_percent = [this](std::string, int value) { return set_air_wb_video_rate_for_mcs_adjustment_percent(value); };
        ret.push_back(Setting{WB_VIDEO_RATE_FOR_MCS_ADJUSTMENT_PERC,
                              openhd::IntSetting{(int)settings.wb_video_rate_for_mcs_adjustment_percent, cb_wb_video_rate_for_mcs_adjustment_percent}});
        ...
        auto cb_dev_air_set_high_retransmit_count = [this](std::string, int value) { return set_dev_air_set_high_retransmit_count(value); };
    ...
    if (any_card_supports_stbc_ldpc_sgi) {
        ...
        auto cb_wb_enable_sg = [this](std::string, int short_gi) {
            if (!validate_yes_or_no(short_gi))
                return false;
    ...
    if (openhd::wb::has_any_rtl8812au(m_broadcast_cards)) {
        auto cb_wb_rtl8812au_tx_pwr_idx_override = [this](std::string, int value) { return request_set_tx_power_rtl8812au(value, false); };
        ...
        auto cb_wb_rtl8812au_tx_pwr_idx_armed = [this](std::string, int value) { return request_set_tx_power_rtl8812au(value, true); };
    ...
    if (openhd::wb::has_any_non_rtl8812au(m_broadcast_cards)) {
        auto cb_wb_tx_power_milli_watt = [this](std::string, int value) { return request_set_tx_power_mw(value, false); };
        ...
        auto cb_wb_tx_power_milli_watt_armed = [this](std::string, int value) { return request_set_tx_power_mw(value, true); };
...
void WBLink::wt_update_statistics() {
    ...
    if (m_profile.is_air) {
        ...
        for (int i = 0; i < m_wb_video_tx_list.size(); i++) {
            ...
            if (curr_tx_stats.n_injected_packets == 0)
                continue;
    ...
    const int tmp_last_management_packet_ts = m_profile.is_air ? m_management_air->get_last_received_packet_ts_ms() : m_management_gnd->get_last_received_packet_ts_ms();
    const int last_received_packet_ts = std::max(m_last_received_packet_ts_ms.load(), tmp_last_management_packet_ts);
    const auto elapsed_since_last_rx_packet_ms = openhd::util::steady_clock_time_epoch_ms() - last_received_packet_ts;
    const bool curr_rx_last_packet_status_good = elapsed_since_last_rx_packet_ms <= 5 * 1000;
    const auto bitfield = openhd::link_statistics::MonitorModeLinkBitfield{(bool)curr_settings.wb_enable_stbc, (bool)curr_settings.wb_enable_ldpc,
                                                                           (bool)curr_settings.wb_enable_short_guard, (bool)curr_rx_last_packet_status_good};
    ...
    for (int i = 0; i < m_broadcast_cards.size(); i++) {
        ...
        if (m_broadcast_cards[i].type == WiFiCardType::OPENHD_RTL_88X2AU || m_broadcast_cards[i].type == WiFiCardType::OPENHD_RTL_88X2BU ||
            m_broadcast_cards[i].type == WiFiCardType::OPENHD_RTL_88X2CU || m_broadcast_cards[i].type == WiFiCardType::OPENHD_RTL_88X2EU ||
        ...
        card_stats.tx_power_current = card.type == WiFiCardType::OPENHD_RTL_88X2AU ? m_curr_tx_power_idx.load() : m_curr_tx_power_mw.load();
        card_stats.tx_power_disarmed = card.type == WiFiCardType::OPENHD_RTL_88X2AU ? curr_settings.wb_rtl8812au_tx_pwr_idx_override : curr_settings.wb_tx_power_milli_watt;
        card_stats.tx_power_armed =
            card.type == WiFiCardType::OPENHD_RTL_88X2AU ? curr_settings.wb_rtl8812au_tx_pwr_idx_override_armed : curr_settings.wb_tx_power_milli_watt_armed;
...
void WBLink::wt_perform_rate_adjustment() {
    ...
    // 计算当前WiFi最大码率
    const int max_rate_for_current_wifi_config = calculate_bitrate_for_wifi_config_kbits(card, settings.wb_frequency, settings.wb_air_tx_channel_width, settings.wb_air_mcs_index,
    ...
    // 该函数用于在无线视频传输或其他通信系统中，计算扣除 FEC（前向纠错）开销后的有效带宽。
    // Subtract the FEC overhead from (video) bitrate
    const int max_video_rate_for_current_wifi_fec_config = openhd::wb::deduce_fec_overhead(max_rate_for_current_wifi_config, settings.wb_video_fec_percentage);
    ...
    if (m_max_video_rate_for_current_wifi_fec_config != max_video_rate_for_current_wifi_fec_config || m_rate_adjustment_frequency_changed) {
        ...
        // called
        m_console->debug("MCS:{} ch_width:{} Calculated max_rate:{}, max_video_rate:{}", settings.wb_air_mcs_index, settings.wb_air_tx_channel_width,
                         openhd::kbits_per_second_to_string(max_rate_for_current_wifi_config), openhd::kbits_per_second_to_string(max_video_rate_for_current_wifi_fec_config));
        m_max_video_rate_for_current_wifi_fec_config = max_video_rate_for_current_wifi_fec_config;
    ...
    if (dropping_many_frames) {
        ...
        if (m_recommended_video_bitrate_kbits < MIN_BITRATE_KBITS) {
            m_console->warn("Reached minimum bitrate {}", openhd::kbits_per_second_to_string(MIN_BITRATE_KBITS));
    ...
    if (OHDPlatform::instance().is_x20()) {
        ...
        const int x20_rate =
            m_thermal_protection_level > 0 ? m_recommended_video_bitrate_kbits * 30 / 100 * factor / 100 : m_recommended_video_bitrate_kbits * 70 / 100 * factor / 100;
...
void WBLink::transmit_video_data(int stream_index, const openhd::FragmentedVideoFrame& fragmented_video_frame) {
    ...
    if (m_thermal_protection_level.load(std::memory_order_relaxed) >= THERMAL_PROTECTION_VIDEO_DISABLED) {
    ...
    if (fragmented_video_frame.dirty_frame != nullptr) {
        // non rtp
        const auto res = tx.try_enqueue_frame(fragmented_video_frame.dirty_frame, max_fec_block_size, fec_perc, fragmented_video_frame.creation_time);
        if (!res) {
        ...
        if (use_dropping_enqueue) {
            const auto count_removed = tx.enqueue_block_dropping(fragmented_video_frame.rtp_fragments, max_fec_block_size, fec_perc, fragmented_video_frame.creation_time);
            if (count_removed != 0) {
                openhd::log::get_default()->debug("Cleared {} frames to make space for frame {}", count_removed, fragmented_video_frame.to_string());
                n_dropped_frames = count_removed;
            }
        } else {
            const auto res = tx.try_enqueue_block(fragmented_video_frame.rtp_fragments, max_fec_block_size, fec_perc, fragmented_video_frame.creation_time);
            if (!res) {
                n_dropped_frames = 1;
                m_console->debug("TX enqueue video frame failed, queue size:{}", tx.get_tx_queue_available_size_approximate());
            }
        }
    }
...
void WBLink::perform_channel_scan(const openhd::LinkActionHandler::ScanChannelsParam& scan_channels_params) {
    ...
    const auto channels_to_scan = openhd::wb::get_scan_channels_frequencies(card, scan_channels_params.channels_to_scan);
    ...
    m_console->debug("Channel scan N channels to scan:{} N channel widths to scan:{}", channels_to_scan.size(), channel_widths_to_scan.size());
    ...
    for (int i = 0; i < channels_to_scan.size(); i++) {
        ...
        for (const auto& channel_width : channel_widths_to_scan) {
            ...
            if (!openhd::wb::any_card_support_frequency(channel.frequency, m_broadcast_cards, m_console)) {
                continue;
            }
            ...
            if (n_likely_openhd_packets > 0) {
                ...
                while (std::chrono::steady_clock::now() - begin_long_listen < std::chrono::seconds(5)) {
                    ...
                    const bool has_received_management = air_center_frequency > 0 && air_tx_channel_width > 0;
            ...
            const int air_center_frequency = m_management_gnd->m_air_reported_curr_frequency;
            const int air_tx_channel_width = m_management_gnd->m_air_reported_curr_channel_width;
            m_console->debug("Got {} packets on {}@{} air_reports:[{}@{}] with loss {}%", n_valid_packets, channel.frequency, channel_width, air_center_frequency,
                             air_tx_channel_width, packet_loss);
...
void WBLink::wt_perform_bw_via_rc_channel_if_enabled() {
    ...
    if (!opt_rc_bw.has_value())
        return;
    const auto rc_bw = opt_rc_bw.value();
    if (settings.wb_air_tx_channel_width != rc_bw) {
        m_console->debug("RC CHANNEL - changing BW from {} to {} ", settings.wb_air_tx_channel_width, rc_bw);
...
void WBLink::wt_gnd_perform_channel_management() {
    if (m_profile.is_ground()) {
        ...
        const int air_reported_frequency = m_management_gnd->m_air_reported_curr_frequency;
        if ((air_reported_channel_width == 20 || air_reported_channel_width == 40) && air_reported_frequency > 100) {
            if (m_gnd_curr_rx_channel_width != air_reported_channel_width || m_gnd_curr_rx_frequency != air_reported_frequency) {
...
void WBLink::on_wifi_card_fatal_error() {
    if (m_wifi_card_error_has_been_handled)
        return;
...
void WBLink::wt_perform_update_thermal_protection() {
    ...
    if (OHDFilesystemUtil::exists(std::string(getConfigBasePath()) + "disable_thermal_limits.txt")) {
