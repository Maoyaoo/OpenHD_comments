...
bool openhd::wb::disable_all_frequency_checks() {
    return OHDFilesystemUtil::exists(std::string(getConfigBasePath()) + "disable_all_frequency_checks.txt");
}

bool openhd::wb::all_cards_support_frequency(uint32_t frequency, const std::vector<WiFiCard>& m_broadcast_cards, const std::shared_ptr<spdlog::logger>& m_console = nullptr) {
...
bool openhd::wb::all_cards_support_frequency_and_channel_width(uint32_t frequency,
...
bool openhd::wb::any_card_support_frequency(uint32_t frequency, const std::vector<WiFiCard>& m_broadcast_cards, const std::shared_ptr<spdlog::logger>& m_console) {
...
bool openhd::wb::set_frequency_and_channel_width_for_all_cards(uint32_t frequency, uint32_t channel_width, const std::vector<WiFiCard>& m_broadcast_cards) {
    ...
    for (const auto& card : m_broadcast_cards) {
        ...
        if (card.type == WiFiCardType::OPENHD_RTL_88X2AU || card.type == WiFiCardType::OPENHD_RTL_88X2BU || card.type == WiFiCardType::OPENHD_RTL_88X2CU ||
            card.type == WiFiCardType::OPENHD_RTL_88X2EU || card.type == WiFiCardType::OPENHD_RTL_8852BU) {
            wifi::commandhelper::openhd_driver_set_frequency_and_channel_width(card.type, card.device_name, frequency, channel_width);
        } else {
...
void openhd::wb::set_tx_power_for_all_cards(int tx_power_mw, int rtl8812au_tx_power_index_override, const std::vector<WiFiCard>& m_broadcast_cards) {
    for (const auto& card : m_broadcast_cards) {
        if (card.type == WiFiCardType::OPENHD_EMULATED) {
        ...
        if (card.type == WiFiCardType::OPENHD_RTL_88X2AU) {
            openhd::log::get_default()->warn("RTL8812AU tx_pwr_idx_override: {}", rtl8812au_tx_power_index_override);
            ...
            const auto tx_power_mbm = openhd::milli_watt_to_mBm(tx_power_mw, adjustment_factor, adjustment_value);
            if (card.type == WiFiCardType::OPENHD_RTL_88X2BU || card.type == WiFiCardType::OPENHD_RTL_88X2CU || card.type == WiFiCardType::OPENHD_RTL_88X2EU ||
                card.type == WiFiCardType::OPENHD_RTL_8852BU || card.type == WiFiCardType::QUALCOMM) {
                wifi::commandhelper::openhd_driver_set_tx_power(card.type, card.device_name, tx_power_mbm);
            } else {
                wifi::commandhelper::iw_set_tx_power(card.device_name, tx_power_mbm);
            }

            openhd::log::get_default()->warn("Tx power mW: {} mBm: {}", tx_power_mw, tx_power_mbm);
        }
    }
}
...
bool openhd::wb::validate_frequency_change(int new_frequency,
                                           int current_channel_width,
                                           const std::vector<WiFiCard>& m_broadcast_cards,
                                           const std::shared_ptr<spdlog::logger>& m_console) {
...
bool openhd::wb::validate_air_channel_width_change(int new_channel_width, const WiFiCard& card, const std::shared_ptr<spdlog::logger>& m_console) {
...
bool openhd::wb::validate_air_mcs_index_change(int new_mcs_index, const WiFiCard& card, const std::shared_ptr<spdlog::logger>& m_console) {
...
int openhd::wb::calculate_bitrate_for_wifi_config_kbits(const WiFiCard& card, int frequency_mhz, int channel_width_mhz, int mcs_index, int dev_adjustment_percent, bool debug_log) {
    // First we calculate the theoretical rate for the current "wifi config" aka
    // taking mcs index, channel width, ... into account
    // 首先根据频率计算 WiFi 的工作频段（例如 2.4 GHz 或 5 GHz）
    const auto wifi_space = openhd::get_space_from_frequency(frequency_mhz);
    // 基于当前 WiFi 配置（设备、频段、MCS 索引、信道宽度）计算最大理论速率（未调整）
    const int max_rate_for_current_wifi_config_without_adjust = get_max_rate_possible(card, wifi_space, mcs_index, channel_width_mhz == 40);
    // 根据设备调整百分比调整最大速率
    ...
    if (debug_log) {
        ...
        m_console->debug("Max rate for {}@{}Mhz MCS:{} dev_adjustment:{} is {} kBit/s", frequency_mhz, channel_width_mhz, mcs_index, dev_adjustment_percent,
