...
bool is_microhard_device_present() {
    if (!OHDFilesystemUtil::exists(std::string(getConfigBasePath()) + "wfb.txt") && !OHDFilesystemUtil::exists(std::string(getConfigBasePath()) + "ethernet.txt")) {
...
OHDInterface::OHDInterface(OHDProfile profile1) : m_profile(std::move(profile1)) {
...
std::vector<openhd::Setting> OHDInterface::get_all_settings() {
    ...
    if (m_wifi_hotspot != nullptr) {
        auto cb_wifi_hotspot_mode = [this](std::string, int value) {
            if (!is_valid_wifi_hotspot_mode(value))
                return false;
        ...
        ret.push_back(openhd::Setting{"WIFI_HOTSPOT_E", openhd::IntSetting{m_nw_settings.get_settings().wifi_hotspot_mode, cb_wifi_hotspot_mode}});
    }
    if (m_profile.is_ground()) {
        ...
        ret.push_back(openhd::Setting{"ETHERNET", openhd::IntSetting{settings.ethernet_operating_mode, cb_ethernet}});
    }
...
void OHDInterface::generate_keys_from_pw_if_exists_and_delete() {
    ...
    if (OHDFilesystemUtil::exists(std::string(getConfigBasePath()) + "password.txt")) {
        auto pw = OHDFilesystemUtil::read_file(std::string(getConfigBasePath()) + "password.txt");
        OHDUtil::trim(pw);
        ...
        if (wb::write_keypair_to_file(keys, openhd::SECURITY_KEYPAIR_FILENAME)) {
            ...
            // delete the file
            OHDFilesystemUtil::remove_if_existing(std::string(getConfigBasePath()) + "password.txt");
    ...
    // 我们会（根据默认密码）一次性创建 txrx.key 文件，以便在后续启动时加快启动速度。
...
void OHDInterface::update_wifi_hotspot_enable() {
    ...
    // 更新 WiFi 热点状态并记录信息
    m_wifi_hotspot->set_enabled_async(enable_wifi_hotspot);                                    // 以异步方式设置 WiFi 热点的启用状态
    openhd::LinkActionHandler::instance().m_wifi_hotspot_state = enable_wifi_hotspot ? 2 : 1;  // 更新 LinkActionHandler 实例中的 WiFi 热点状态，启用时为 2，禁用时为 1
    openhd::LinkActionHandler::instance().m_wifi_hotspot_frequency = m_wifi_hotspot->get_frequency();  // 更新 LinkActionHandler 实例中的 WiFi 热点频率
}
